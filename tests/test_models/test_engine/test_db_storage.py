...
class TestDBStorage(unittest.TestCase):
    ...
    @unittest.skipIf(models.storage_t != 'db', "not testing db storage")
    def test_get(self):
        """"Test that get method retrieves object successfully"""
        new_state = State(name="New York")
        new_state =.save()
        new_user = User(email="antony@foobar.com", password="password")
        new_user.save()
    ...
    @unittest.skipIf(models.storage_t != 'db', "not testing db storage")
    def test_count(self):
        """Test that the count method counts the number of objects in storage"""
        initial_count = models.storage.count()
        self.assertEqual(models.storage.count("Blah"), 0)
        new_state = State(name="Florida")
        new_state.save()
        new_user = User(email="antony@foobar.com", password="passowrd")
        new_user.save()
